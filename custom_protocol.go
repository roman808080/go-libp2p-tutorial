...
func NewCustomService(h host.Host) *CustomService {
	ps := &CustomService{h}
	h.SetStreamHandler(ID, ps.CustomHandler)
	return ps
}

func (p *CustomService) CustomHandler(s network.Stream) {
...
func (ps *CustomService) Ping(ctx context.Context, p peer.ID) <-chan Result {
...
func Ping(ctx context.Context, h host.Host, p peer.ID) <-chan Result {
	...
	go func() {
		...
		for ctx.Err() == nil {
			var res Result
			res.RTT, res.Error = custom_ping(s, ra)
...
func custom_ping(s network.Stream, randReader io.Reader) (time.Duration, error) {
